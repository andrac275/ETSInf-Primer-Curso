...
public class RecursioNumeros {
     ...
     *  Si a<b, a/b = 0.
     *  Si a>=b, a/b = (a-b)/b + 1. Se suma 1 perque es "el cas següent" en la divisio.
     *  PRECONDICIO: a >= 0 i b > 0
     *  
     *  a) enunciat problema
     *  b) analisi per casos
     *  c) trancripcio algorismica
     *  d) verificacio
     */
    ...
    public static double potenciaM(double a, int b) { 
        /* COMPLETAR */
        //Este es el meu. Funciona pero es poc eficient. Recorda Fibonacci i el que consumeix.
        // double res;
        // if (b == 0) {res = 1;}
        // else if (b == 1) { res = a;}
        // else {
            // if (b % 2 == 0) {res = potenciaM(a, b / 2) * potenciaM(a, b / 2);}
            // else {res = potenciaM(a, b / 2) * potenciaM(a, b / 2) * a;}
        // }
        
        // return res;
        
        //Este es mes eficient.
        double res;
        if (b == 0) {res = 1;}
        else if (b == 1) { res = a;}
        else {
            double operacio = potenciaM(a, b / 2);
            res = operacio * operacio;
            if (b % 2 == 1) {res = res * a;}
    ...
    public static int nombreDigits(int n) {
        ...
        if(n < 10) {res = 1;}
            ...
            res = 1 + nombreDigits(n / 10);
        }
        return res;
    }
    
    /** Mostra per pantalla l'invers de n.
     * Ex: Si n= 1234 torna 4321
     *  PRECONDICIO: n >= 0 */
    public static void mostraInvers(int n) {
        /* COMPLETAR */
        if (n < 10){System.out.println (n);} //Este te salto de linia perque nomes hi ha un nombre.
        else {
            System.out.print(n % 10);   //Aquest no te salt linia perque te mes d'un nombre i el que se vol es que ho escriga a continuacio.
            mostraInvers(n / 10);
        }
    }
     ...
     * Ex: Si n= 1234 torna 4321
     *  PRECONDICIO: n >= 0 */
    public static int tornaInvers(int n) {
        /* COMPLETAR */
        int res;
        if (n < 10){res = n;}
        else {
            int zeros = (int) Math.pow(10, (int) Math.log10(n));
            res = ((n % 10) * zeros) + tornaInvers(n / 10);
        }
        return res;
    }
