...
public class Grup {
    ...
    private double notaMinima(int pos) {
        /* COMPLETAR */
        //1. Identificar cas base i instruccions.
        if (pos == numAl - 1){
            //Si estic ja en el ultim alumne, la nota minima es la nota deixe alumne.
            return grup[pos].getNota();
        } 
        //2. Recursio + Instruccions (Math.min(....))
        else {
            //Si no... La nota minima es el minim entre la posicio en la que estem 
            // "i tot el demes"; que es la cridà recursiva.
            return Math.min(grup[pos].getNota() , notaMinima(pos + 1));
        }
    }
     ...
     *  PRECONDICIO: -1 <= pos < numAl 
        Si te un 10 de nota, es que te una matricula d'honor.
        */
